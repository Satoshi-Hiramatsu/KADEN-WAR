...
**現在のリポジトリ版: V0.0.2 / S0開発基盤を実装中（未コミット）。ゲーム本体は未実装です。**

ローカルReact画面、暦表示PoC、Workers/D1疎通API、ビルド・テストとCI設定を追加しました。週送りと接続確認を試せます。製品開発・販売・決算・保存と公開URLはまだありません。S0の実環境配信確認は残っています。
...
| 実装済み | S0検証画面、暦の導出、D1疎通API・ローカルマイグレーション |
| 未実装 | 経営エンジン、担当別画面、シナリオデータ、保存・認証、クラウド配信 |
...
React・Vite・TypeScript・Vitest・Wranglerを導入し、依存をロックファイルに固定しました。ZustandはS1用に導入済みですがS0画面はReactのローカル状態を使用。Canvas・IndexedDB・ゲーム状態管理は後続工程です。基本プレイはローカルで完結し、クラウド保存は任意同期とする設計です。
...
Node.js 24系で `npm ci`、`npm run api:types`、`npm run dev` を実行すると `http://127.0.0.1:5173` で暦の検証画面を開けます。接続確認も使う場合は別ターミナルで `npm run db:migrate`、`npm run api:dev` を実行します。検証は `npm run check`。

[開発・検証手順](docs/development.md)にAPIと配信の残作業、[ルールノート](docs/rule-note.md)に採用仕様と未決事項を記載しています。

追加構成は `apps/web/`（React）、`apps/api/`（Workers/D1）、`packages/simulation/`（暦）、`packages/content/`（暦定義）、`packages/contracts/`（API型）、`docs/`、`.github/workflows/`。以下は既存の設計・素材構成です。
...
S0基盤の検証結果は[更新履歴](CHANGELOG.md)に記録します。ゲームの決算・保存・シナリオ完走・端末別受入は未実施です。
